...
//	constexpr bool output_type_selection	= MICRO_VOLT;
	...
	std::vector<uint16_t>	registers_list	= { 0x80, 0x90 };

	printf( "=== before overwrite ===\r\n" );
	register24_dump( registers_list );

	//
	//	checking offset with default gain/offset coefficients
	//		this takes >6sec for each gain setting
	//

#if 0
	const std::vector<float>	gain_setting	= { 0.2, 0.4, 0.8, 1, 2, 4, 8, 16 };
	int32_t	sum	=	0;
	constexpr auto	length	= 100;

	for ( int g = 0; g < 8; g++ )
	{
		uint16_t	ch_config0	= 0x0010 | (g << 5);
		afe.logical_ch_config( 0, ch_config0, 0xF084, 0x2900, 0x0000 );
		printf( "ch_config0 = %04X, gain = %3.1f", ch_config0, gain_setting[ g ] );
		wait( 1.0 );

		for ( int i = 0; i < length; i++ )
		{
			sum	+= afe.read<raw_t>( 0, read_delay );
			wait( 0.05 );
		}
		printf( " : result = %11.2lf\r\n", (double)sum / (double)length );
	}
#endif
